...
function show_results() {
  ...
  for ($i=0; $i<count($rows); $i++) {
    ...
    $out[] =	"    <td>".($rows[$i]['percent'] ? "<img align='middle' border='1' bordercolor='#808080' src='../assets/ff0000.gif' height='".poll_column_height."' width='".((int)$rows[$i]['percent']*poll_column_width/100)."' title='(".$rows[$i]['votes']." votes)'>" : "")." (".$rows[$i]['percent']."%)</td>\n";
    $out[] =	"  </tr>";
  }
  $out[] =	"  <tr>\n";
...
function poll_edit() {
  ...
  $out[] =	"              <tr>\n";
  $out[] =	"                <td width='18'><img src='../assets/corner_top_left.gif' width='15' height='18'></td>\n";
  $out[] =	"                <td width='100%' class='downloadTableHeadings_nosort'>Poll Details</td>\n";
  $out[] =	"                <td width='18'><img src='../assets/corner_top_right.gif' width='15' height='18'></td>\n";
  $out[] =	"              </tr>\n";
  $out[] =	"            </table>\n";
...
function poll_list() {
  ...
  if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
    $out[] =	"    <th valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" class='downloadTableHeadings' title='Sort by Active status' onclick=\"document.form.sortBy.value='active".($sortBy=="active" ? "_d" : "")."';document.form.submit()\" align='left'>Active ".($sortBy=='active' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='active_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n";
  }
  $out[] =	"  </tr>\n";
  $out[] =	"  </thead>\n";
  ...
  for ($i=0; $i<mysql_num_rows($result); $i++) {
    ...
    $out[] =	"</table></td>";
    if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
      $out[] =	"    <td valign='top'><a href='javascript:document.form.submode.value=\"activate\";document.form.ID.value=\"".$row['ID']."\";document.form.submit();'><img src='../assets/checkbox_".$row['active'].".gif' border='0'></a></td>";
    }
    $out[] =	"  </tr>\n";
  }
  $out[] =	"</table><br><br>";
