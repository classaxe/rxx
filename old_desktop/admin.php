...
function admin_manage() {
  ...
  $out =
     "<form name='form' METHOD='POST' ENCTYPE='multipart/form-data' ACTION='$script'>\n"
    ."<input type='hidden' name='mode' value='$mode'>\n"
    ."<input type='hidden' name='submode' value=''>\n"
    ."<input type='hidden' name='MAX_FILE_SIZE' value='800000'>\n"
    ."<h1>Administrator Management Tools</h1><br>\n<ol class='p'>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_signalUpdateFromGSQ\"'> <b>Signals: Update Lat and Lon values from GSQ</b> (use after importing new signals using phpMyAdmin)</li>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_signalUpdateLogCount\"'> <b>Signals: Update log counts and 'Heard In' lists</b> (run periodically)</li>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_signalUpdateRegionsHeard\"'> <b>Signals: Update all for regions heard and heard_in lists</b> (run if problems are seen)</li><br><br>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_logsUpdateDX\"'> <b>Logs: Recalculate all distances</b> (use after adding GSQs for existing logging or function above)</li>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_setDaytimeLogs\"'> <b>Logs: Mark daytime loggings</b> (run periodically)</li><br><br>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_listenersUpdateLogCount\"'> <b>Listeners: Update log counts</b> (run periodically)</li><br><br>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='this.disabled=1;document.location=\"$script?mode=$mode&submode=admin_importICAO\"'> <b>ICAO: Get latest data</b> (run once a month)</li><br><br>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='document.location=\"$script?mode=db_export\"'> <b>System: Export Database</b> (run periodically)</li>\n"
    ."<li><input type='button' class='formButton' value='Go' onclick='popWin(\"http://www.classaxe.com/phpMyAdmin/?pma_username=ndb-ndbrna&pma_password=kj356y9945m\",\"popMysql\",\"scrollbars=1,resizable=1,toolbar=1,location=0\",800,600,\"\");'> <b>System: Use phpMyAdmin</b> (used to directly alter all data)</li>\n"
  ...
  switch ($submode) {
    ...
    case "db_backup":				        $out.= "<h1>Create System Restore Point</h1><br>".db_dump();	break;
    case "admin_dbRestore":			        $out.= "<h1>Restore using System Restore Point</h1><br>".admin_dbRestore(); break;
  }
  return $out;
}
...
function admin_dbRestore() {
  ...
  global $mode, $submode, $script, $file;
  $out = array();
  ...
  if (!$file) {
    ...
    $out[] =	"<form name='form' action='$script?mode=$mode&submode=$submode' method='POST' onsubmit='this.go.disabled=1'>\n";
    $out[] =	"<input type='hidden' name='mode' value='$mode'>\n";
    $out[] =	"<input type='hidden' name='submode' value='$submode'>\n";
    $out[] =	"<select name='file' size='15' class='formField'>\n";

    for ($i=0; $i<count($files); $i++) {
      $out[] =	"<option value='".$files[$i].".sql'>$files[$i]</option>\n";
    }
    $out[] =	"</select>\n";
    $out[] =	"<input type='submit' value='Go' name='go'>\n";
    $out[] =	"</form>\n";
  }
  else {
...
//  print("<pre>$sql</pre>");
  ...
  return "<h1>Updating DX values</h1><br><br><p>Done. $updated logs updated</p>";
}
...
function admin_importICAO() {
  global $script, $mode, $submode, $subsubmode, $data;
  $out =	array();
  ...
  $out[] =	"<input type='hidden' name='mode' value='$mode'>\n";
  $out[] =	"<input type='hidden' name='submode' value='$submode'>\n";
  $out[] =	"<input type='hidden' name='subsubmode' value=''>\n";
  $out[] =	"<h1>Import ICAO</h1><br><br>\n";
  ...
  $out[] =	"      <tr>\n";
  $out[] =	"        <th align='left' class='downloadTableHeadings_nosort'><a name='dx'></a>ICAO Data</th>\n";
  $out[] =	"        <th align='right' class='downloadTableHeadings_nosort'><small>[ <a href='#top' class='yellow'><b>Top</b></a> ]</small></th>\n";
  $out[] =	"      </tr>\n";
  $out[] =	"    </table></th>\n";
  $out[] =	"  </tr>\n";
  ...
  if ($subsubmode=="Update") {
    ...
    if ($data[3] == "\n---CONNECTION ERROR---") {
      ...
      $out[] =	"  <tr>\n";
      $out[] =	"    <td class='downloadTableContent'>Success: Database updated, $num records processed</td>";
      $out[] =	"  </tr>\n";
    }
  }
  else {
    $out[] =	"  <tr>\n";
    $out[] =	"    <td class='downloadTableContent'><textarea class='formFixed' cols='55' rows='30' name='data'>";
    $out[] =	"Data extracted from\n$url\n\n";
    ...
    if ($my_file = @file($url)) {
      ...
      $out[] =	"  </tr>\n";
      $out[] =	"  <tr>\n";
      $out[] =	"    <td class='downloadTableContent' align='center'><input type='submit' name='go' value='Update' class='formButton' onclick='document.form.subsubmode.value=\"Update\";document.form.go.disabled=true;document.form.submit()'></td>";
      $out[] =	"  </tr>\n";
    }
    else {
...
function admin_signalUpdateLogCount() {
  ...
  for ($i=0; $i<mysql_num_rows($result); $i++) {
    ...
    $sql =	"SELECT COUNT(*) as `logs` FROM `logs` WHERE `signalID` = ".$ID." AND `listenerID` != ''";
  ...
  return "<h1>Updating Log Counts...</h1><br><br><p>Done. $updated signals updated.</p>";
}
...
function admin_signalUpdateFromGSQ() {
  ...
  return "<h1>Updating Signal Locations...</h1><br><br><p>Done. $updated signals updated.</p>";
}
...
function admin_listenersUpdateLogCount() {
  ...
  return "<h1>Updating Log Counts...</h1><br><br><p>Done. $updated Listeners updated.</p>";
}
...
function admin_systemSendTestEmail() {
  ...
  if ($mail->Send()) {
    return "<h1>Sent test email to $sendToEmail via ".smtp_host."</h1>";
  }
  return "<h1>Test email to $sendToEmail via ".smtp_host." failed.</h1>";
}
...
function sys_info() {
  ...
  phpinfo();
  $tmp = ob_get_contents();
  $tmp = str_replace(" width=\"600\"", " style=\"whitespace:wrap\" width=\"600\"", $tmp);
  ob_end_clean();
...
//  return $tmp;
  ...
  return
	...
	."<tr><td class=\"e\">system_backup</td><td class=\"v\">".system_backup."</td></tr>\n"
	."</table>\n"
	."<div id='phpinfo'>\n"
    ."<hr />\n"
    ."<h1>PHP INFO</h1>\n"
...
// ************************************
function log_upload() {
  global $script, $mode, $submode, $log_format, $log_entries, $log_dd, $log_mm, $log_yyyy, $listener_in, $listener_timezone;
  ...
  $debug = 0;
  $out =	array();
  $out[] =	"<form name='form' action='$script?mode=$mode' method='POST'>";
  ...
  switch ($submode) {
    case "":
      ...
      $out[] =
         "<h1>Add Log > Parse Data</h1><br><img src='../assets/spacer.gif' height='4' width='1'>"
    ...
    case "parse_log":
      ...
      $out[] =	"<h1>Add Log > Confirm Data</h1><br><img src='../assets/spacer.gif' height='4' width='1'>";
    ...
    case "submit_log":
      ...
      $result = mysql_query($sql);
      $row =	mysql_fetch_array($result,MYSQL_ASSOC);
      $out[] =	"<h1>Add Log > Results</h1><br><img src='../assets/spacer.gif' height='4' width='1'>";
      $out[] =	"<table cellpadding='2' cellspacing='1' border='0' bgcolor='#c0c0c0'>\n";
      $out[] =	"  <tr>\n";
      $out[] =	"    <th colspan='4' class='downloadTableHeadings_nosort'>Listener Details</th>\n";
      $out[] =	"  </tr>\n";
...
//              print("<pre>$sql</pre>");
  ...
  if ($listenerID!="" && $submode=="") {
    $out[] =
      ...
      ."  </tr>\n"
      ."  <tr class='rownormal'>\n"
      ."    <td><input name='log_format' class='fixed_heading' size='84' value='$log_format'><input class='formbutton' name='save' type='button' value='Save' onclick='this.disabled=true;document.form.go.disabled=true;document.form.conv.disabled=true;document.form.submode.value=\"save_format\";document.form.submit()'>"
      ."  </tr>\n"
      ."  <tr class='rownormal'>\n"
      ."    <td><textarea rows='20' cols='90' class='fixed' name='log_entries' onKeyUp='check_for_tabs(document.form);' onchange='check_for_tabs(document.form);'>".stripslashes($log_entries)."</textarea>\n"
      ."  </tr>\n";
    ...
    $out[] =
      ...
      ."  for (idx in log_arr) {\n"
      ."    line = log_arr[idx].replace(/^\s+|\s+$/g,'').replace(/\s+/g,' ');\n" // trims ends and converts multiple spaces to singles
...
// ************************************
function logon() {
  global $script,$server,$mode,$submode;
  global $user,$password;
  if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
    return 	 "<h1>Logon</h1><p>You are now logged on as an Administrator and may perform administrative functions.<br><br>\nTo log off, select <b>Log Off</b> from the main menu.</p>";
  }
  return "<h1>Logon</h1><p>You must logon in order to perform administrative functions.</p>"
