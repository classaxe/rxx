...
class Signal extends Record{
  ...
  function tabs() {
    ...
    $out =
        "                ".tabItem("Profile","signal_info",50);
    if ($signal) {
      if ($signal['logs']) {
        $out.=
...
function signal_dgps_messages() {
  ...
  if (count($dgps_messages)) {
    $out[] =
      ...
      ."&nbsp;DGPS Messages for ".(float)$khz."-".$call.($QTH ? ", $QTH" : "").($SP ? ", $SP" : "").($ITU ? ", $ITU" : "")."</th>\n"
      ."              </tr>\n"
      ."              <tr>\n"
      ."                <td bgcolor='white'><table cellpadding='0' cellspacing='1' border='0' class='noprint'>\n"
      ."                  <thead>\n"
      ."                  <tr>\n"
      ."                    <th class='scroll_list' width='90' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=`attachment`.`type`,`attachment`.`title`".($sortBy=='`attachment`.`type`,`attachment`.`title`' ? 'DESC' : '')."'>".($sortBy=='`attachment`.`type`,`attachment`.`title`'||$sortBy=='`attachment`.`type`,`attachment`.`title`DESC'?'<font color="#ff0000">Type</font>':'Type')."</a></small></th>\n"
      ."                    <th class='scroll_list' width='220' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=`description`".($sortBy=='`description`' ? 'DESC' : '')."'>".($sortBy=='`description`'||$sortBy=='`description`DESC'?'<font color="#ff0000">Transmission Times</font>':'Transmission Times')."</a></small></th>\n"
      ."                  </tr>\n"
      ."                  </thead>\n"
      ."                </table></td>\n"
      ."              </tr>\n"
      ...
      ."                  <thead>\n"
      ."                  <tr class='noscreen'>\n"
      ."                    <th class='scroll_list' width='90' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=type".($sortBy=='type' ? '_d' : '')."'>".($sortBy=='type'||$sortBy=='type_d'?'<font color="#ff0000">Type</font>':'Type')."</a></small></th>\n"
      ."                    <th class='scroll_list' width='220' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=description".($sortBy=='description' ? '_d' : '')."'>".($sortBy=='description'||$sortBy=='description_d'?'<font color="#ff0000">Transmission Times</font>':'Transmission Times')."</a></small></th>\n"
      ."                  </tr>\n"
      ."                  </thead>\n"
      ."                </table>\n"
      ."                <table cellpadding='0' cellspacing='1' border='0' bgcolor='#ffffff'>\n"
    ...
    for ($i=0; $i<count($dgps_messages); $i++) {
      $row =	$dgps_messages[$i];
      $out[] =
         "                  <tr>\n"
        ."                    <td class='scroll_list' width='90' valign='top'><b>".$row["title"]."</b></td>"
        ."                    <td class='scroll_list' width='220' valign='top'>".$row["description"]."</td>\n"
        ."                  </tr>\n";
    }
    $out[] =
       "                  </div></table></td>\n"
...
//      return	"<script language='javascript' type='text/javascript'>window.setTimeout('window.opener.location.reload(1);window.close()',500)</script>";
  ...
  $out =
    ...
    .(isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session ?
        "            <form action='$script' name='form' method='POST'>\n"
       ."            <input type='hidden' name='ID' value='$ID'>\n"
       ."            <input type='hidden' name='mode' value='$mode'>\n"
    ...
    ."              <tr>\n"
    ."                <td width='18'><img src='../assets/corner_top_left.gif' width='15' height='18'></td>\n"
    ."                <td width='100%' class='downloadTableHeadings_nosort'>Profile</td>\n"
    ."                <td width='18'><img src='../assets/corner_top_right.gif' width='15' height='18'></td>\n"
    ."              </tr>\n"
    ."            </table>\n"
    ...
    ."                  <tr>\n"
    ."                    <td><input type='text' size='6' maxlength='12' name='call' value='$call' class='formfield'></td>\n"
  ...
  if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
    $out.=
      ...
      ."<option value='".DGPS."'".($type==DGPS ? " selected" : "")." style='background-color: #00D8ff;'>DGPS</option>\n"
      ."<option value='".TIME."'".($type==TIME ? " selected" : "")." style='background-color: #FFE0B0;'>Time</option>\n"
      ."<option value='".NAVTEX."'".($type==NAVTEX ? " selected" : "")." style='background-color: #FFB8D8;'>NAVTEX</option>\n"
      ."<option value='".HAMBCN."'".($type==HAMBCN ? " selected" : "")." style='background-color: #B8FFC0;'>Amateur Beacon</option>\n"
      ."<option value='".OTHER."'".($type==OTHER ? " selected" : "")." style='background-color: #B8F8FF;'>Other</option>\n"
      ."</select>\n"
      ;
  }
  else {
  ...
  $out.=
    ...
    ."                <td><input type=\"text\" size=\"2\" maxlength=\"2\" name=\"SP\" value=\"$SP\" class=\"formfield\"><span title='State or Province'> <a href='javascript:show_sp()' title='NDBList State and Province codes'><b>(List)</b></a></span></td>\n"
    ."                <th align='right'>Country</th>\n"
    ."                <td><input type=\"text\" size=\"3\" maxlength=\"3\" name=\"ITU\" value=\"$ITU\" class=\"formfield\">".(isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session ? "" : " (".get_ITU($ITU).")")." <span title='Country Codes'><a href='javascript:show_itu()' title='NDBList Country codes'><b>(List)</b></a></span></td>\n"
    ."              <tr class='rowForm'>\n"
    ."                <th align='left'>GSQ</th>\n"
    ."                <td colspan='3'><table cellpadding='0' cellspacing='0' border='0' width='100%'>\n"
    ."                  <tr class='rowForm'>"
    ."                    <td><input type='text' size='6' maxlength='6' name='GSQ' value='$GSQ' class='formfield'> [ "
    ."<a onmouseover='window.status=\"Use coordinate converter\";return true;' onmouseout='window.status=\"\";return true;' href=\"javascript:popWin('./?mode=tools_coordinates_conversion&GSQ=$GSQ','tools_coordinates_conversion','scrollbars=0,resizable=0',610,144,'centre')\" title='Use coordinate converter'><b>Convert</b></a>"
    .(isset($row) ?
        " | <a onmouseover='window.status=\"View Mapquest map for ".(float)$row["khz"]."-".$row["call"]."\";return true;' onmouseout='window.status=\"\";return true;' href='javascript:popup_map(\"".$row["ID"]."\",\"".$row["lat"]."\",\"".$row["lon"]."\")' title='Show Mapquest map\\\n(accuracy limited to nearest Grid Square)'><b>Mapquest</b></a>"
       ." | <a onmouseover='window.status=\"View Google map for ".(float)$row["khz"]."-".$row["call"]."\";return true;' onmouseout='window.status=\"\";return true;' href='javascript:popup_googlemap(\"".$row["ID"]."\",\"".$row["lat"]."\",\"".$row["lon"]."\")' title='Show Google map\\\n(accuracy limited to nearest Grid Square)'><b>Google</b></a>"
      : ""
     )
    ." ]</td>\n"
...
function signal_list() {
  global $script, $mode, $submode, $targetID, $filter_active, $filter_date_1, $filter_date_2;
  ...
  switch ($filter_system) {
    ...
    break;
    case "not_logged":
      $filter_system_SQL =			 "(`heard_in_af` = 0 AND `heard_in_as` = 0 AND `heard_in_ca` = 0 AND `heard_in_eu` = 0 AND\n"
...
//  print("<pre>$sql</pre>");
  ...
  $sql =	"select COUNT(*) AS `signals` from `signals` WHERE ("
  ...
  $sql =	"select COUNT(*) AS `signals` from `signals` WHERE (`logs`>0)";
  $result = 	mysql_query($sql);
  $row =	mysql_fetch_array($result,MYSQL_ASSOC);
  $RWW =	$row["signals"];
  ...
  $sql =	"select COUNT(*) AS `logs` from `logs` WHERE $filter_system_listener_log_SQL";
  $result = 	@mysql_query($sql);
  $row =	mysql_fetch_array($result,MYSQL_ASSOC);
  $logs =	$row["logs"];
  ...
  switch ($sortBy) {
    ...
    case "format":		$sortBy_SQL =	"`active` DESC, `format`='' OR `format` IS NULL, `format` ASC";		break;
    ...
    case "sec":			$sortBy_SQL =	"`active` DESC, `sec`='' OR `sec` IS NULL, `sec` ASC";			break;
    case "sec_d":		$sortBy_SQL =	"`active` DESC, `sec`='' OR `sec` IS NULL, `sec` DESC";			break;
  ...
  $sql =	 "SELECT\n"
		.($filter_heard || $listenerID ?
		   ($listenerID ? "DISTINCT `signals`.*, `logs`.`dx_km`, `logs`.`dx_miles`\n" : "DISTINCT `signals`.*")
		   .($filter_dx_gsq ?	 ",\n  round(degrees(acos(sin(radians($filter_dx_lat)) * sin(radians(signals.lat)) + cos(radians($filter_dx_lat)) * cos(radians(signals.lat)) * cos(radians($filter_dx_lon - signals.lon))))*69, 2) AS `range_dx_miles`,\n"
					."     round(degrees(acos(sin(radians($filter_dx_lat)) * sin(radians(signals.lat)) + cos(radians($filter_dx_lat)) * cos(radians(signals.lat)) * cos(radians($filter_dx_lon - signals.lon))))*111.05, 2) AS `range_dx_km`"
					: "")
		   ."\nFROM\n  `signals`,\n  `logs`\nWHERE\n  `signals`.`ID` = `logs`.`signalID` AND\n$filter_system_SQL ".($filter_heard ? "AND\n ($filter_heard_SQL)" : "").($listenerID ? "AND\n(`logs`.`listenerID`=".implode($listenerID," OR `logs`.`listenerID`=").") " : "") :
                   " `signals`.*"
		   .($filter_dx_gsq ? ",\n  round(degrees(acos(sin(radians($filter_dx_lat)) * sin(radians(signals.lat)) + cos(radians($filter_dx_lat)) * cos(radians(signals.lat)) * cos(radians($filter_dx_lon - signals.lon))))*69, 2) AS `range_dx_miles`,\n  round(degrees(acos(sin(radians($filter_dx_lat)) * sin(radians(signals.lat)) + cos(radians($filter_dx_lat)) * cos(radians(signals.lat)) * cos(radians($filter_dx_lon - signals.lon))))*111.05, 2) AS `range_dx_km`" : "")
		   ."\nFROM\n  `signals`\nWHERE\n  ".$filter_system_SQL)
		.($filter_active ? " AND\n  `active` = 1" : "")
...
//die("<pre>$sql</pre>");
  ...
  $out.=
	...
	."<table cellpadding='0' cellspacing='0' border='0'><tr><td><h1>Signal List</h1><br>\n"
	...
	."<li>This report prints best in Landscape.</li></td></tr></table>\n";
  if ($type_NDB) {
    $out.=
  ...
  if ($type_DGPS) {
	...
	."[ <a href='http://www.beaconworld.org.uk/files/worldDGPSfreqorder.pdf' target='_blank'><b>NDB List PDF (by Frequency)</b></a> | <a href='http://www.beaconworld.org.uk/files/worldDGPScountryorder.pdf' target='_blank'><b>NDB List PDF (by Country)</b></a> | <a href='http://www.navcen.uscg.gov/ado/DgpsCompleteConfiguration.asp' target='_blank'><b>USCG DGPS Site List</b></a> ]</td></tr></table>\n";
  }
  ...
  $out.=
	...
	."      <tr>\n"
	."        <td width='18'><img src='../assets/corner_top_left.gif' width='15' height='18' class='noprint'></td>\n"
	."        <td width='100%' class='downloadTableHeadings_nosort' align='center'>Customise Report</td>\n"
	."        <td width='18'><img src='../assets/corner_top_right.gif' width='15' height='18' class='noprint'></td>\n"
	."      </tr>\n"
	."    </table>\n"
	...
	."        <td nowrap><table cellpadding='0' cellspacing='0' border='0' width='100%'>\n"
	."          <tr>\n"
	."            <td nowrap>&nbsp;<span title='List of States or Provinces'><a href='javascript:show_sp()' title='NDBList State and Province codes'><b>States</b></a></span> <input title='Enter one or more states or provinces (e.g. MI or NB) to show only signals physically located there' type='text' name='filter_sp' size='20' value='$filter_sp' class='formfield'></nobr></td>\n"
	."            <td nowrap align='right'><span title='List of Countries'><a href='javascript:show_itu()' title='NDBList Country codes'>&nbsp;<b>Countries</b></a></span> <input title='Enter one or more NDBList approved 3-letter country codes (e.g. CAN or BRA) to show only signals physically located there' type='text' name='filter_itu' size='20' value='$filter_itu' class='formfield'></nobr></td>"
	."          </tr>\n"
	."	 </table></td>"
	."      </tr>\n"
	."      <tr class='rowForm'>\n"
  ...
  if (mysql_num_rows($result)) {
	...
	."  <thead>\n"
	."  <tr id=\"header\">\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='khz".($sortBy=="khz" ? "_d" : "")."';document.form.submit()\" title=\"Sort by Frequency\">KHz ".($sortBy=='khz' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='khz_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='call".($sortBy=="call" ? "_d" : "")."';document.form.submit()\" title=\"Sort by Callign or DGPS Station ID\">ID ".($sortBy=='call' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='call_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	.($type_NDB ?
	   "    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='LSB".($sortBy=="LSB" ? "_d" : "")."';document.form.submit()\" title=\"Sort by LSB (-ve Offset)\">LSB ".($sortBy=='LSB' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='LSB_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	  ."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='USB".($sortBy=="USB" ? "_d" : "")."';document.form.submit()\" title=\"Sort by USB (+ve Offset)\">USB ".($sortBy=='USB' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='USB_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	  ."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='sec".($sortBy=="sec" ? "_d" : "")."';document.form.submit()\" title=\"Sort by cycle duration\">Sec ".($sortBy=='sec' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='sec_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	  ."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='format".($sortBy=="format" ? "_d" : "")."';document.form.submit()\" title=\"Sort by cycle format\">Fmt ".($sortBy=='format' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='format_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	 : ""
	)
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='QTH".($sortBy=="QTH" ? "_d" : "")."';document.form.submit()\" title=\"Sort by 'Name' and Location\">'Name' and Location ".($sortBy=='QTH' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='QTH_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='sp".($sortBy=="sp" ? "_d" : "")."';document.form.submit()\" title=\"Sort by State / Province\">S/P ".($sortBy=='sp' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='sp_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='itu".($sortBy=="itu" ? "_d" : "")."';document.form.submit()\" title=\"Sort by NDB List Country Code\">ITU ".($sortBy=='itu' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='itu_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='gsq".($sortBy=="gsq" ? "_d" : "")."';document.form.submit()\" title=\"Sort by GSQ Grid Locator Square\">GSQ ".($sortBy=='gsq' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='gsq_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='".($sortBy=="pwr_d" ? "pwr" : "pwr_d")."';document.form.submit()\" title=\"Sort by Transmitter Power\">PWR ".($sortBy=='pwr' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='pwr_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='notes".($sortBy=="notes" ? "_d" : "")."';document.form.submit()\" title=\"Sort by 'Notes' column\">Notes ".($sortBy=='notes' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='notes_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='heard_in".($sortBy=="heard_in" ? "_d" : "")."';document.form.submit()\" title=\"Sort by 'Heard In' column\">Heard In <span style='font-weight: normal'>(Click for Map - <b>bold</b> = daytime logging)</span>".($sortBy=='heard_in' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='heard_in_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='logs".($sortBy=="logs" ? "_d" : "")."';document.form.submit()\" title=\"Sort by 'Logs' column\" nowrap>Logs ".($sortBy=='logs' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='logs_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
	."    <th class='downloadTableHeadings' rowspan='2' align='left' valign='bottom' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='last_heard".($sortBy=="last_heard" ? "_d" : "")."';document.form.submit()\" title=\"Sort by 'Last Heard' column (YYYY-MM-DD)\" nowrap>Last Heard ".($sortBy=='last_heard' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='last_heard_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n";

    if ($listenerID) {
      $out.=	"    <th class='downloadTableHeadings_nosort' colspan='2'>DX from<br>Listener</th>\n";
    }
    if ($filter_dx_gsq) {
      $out.=	"    <th class='downloadTableHeadings_nosort' colspan='2'>DX from<br>GSQ</th>\n";
    }
    if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
    ...
    if ($listenerID || $filter_dx_gsq) {
      $out.=	"  <tr>\n";
      if ($listenerID) {
        $out.= "    <th class='downloadTableHeadings' align='left' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='".($sortBy=="dx_d" ? "dx" : "dx_d")."';document.form.submit()\" title=\"Sort by 'KM' column\"  nowrap>KM ".($sortBy=='dx' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='dx_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
		."    <th class='downloadTableHeadings' align='left' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='".($sortBy=="dx_d" ? "dx" : "dx_d")."';document.form.submit()\" title=\"Sort by 'Miles' column\" nowrap>Miles ".($sortBy=='dx' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='dx_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n";
      }
      if ($filter_dx_gsq) {
        $out.= "    <th class='downloadTableHeadings' align='left' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='".($sortBy=="range_dx_km_d" ? "range_dx_km" : "range_dx_km_d")."';document.form.submit()\" title=\"Sort by 'Range KM' column\" nowrap>KM ".($sortBy=='range_dx_km' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='range_dx_km_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n"
		."    <th class='downloadTableHeadings' align='left' onmouseover=\"column_over(this,1);\" onmouseout=\"column_over(this,0);\" onmousedown=\"column_over(this,2);\" onclick=\"document.form.sortBy.value='".($sortBy=="range_dx_km_d" ? "range_dx_km" : "range_dx_km_d")."';document.form.submit()\" title=\"Sort by 'Range Miles' column\" nowrap>Miles ".($sortBy=='range_dx_km' ? "<img src='../assets/icon_sort_asc.gif' alt='A-Z'>" : "").($sortBy=='range_dx_km_d' ? "<img src='../assets/icon_sort_desc.gif' alt='Z-A'>" : "")."</th>\n";
      }
      $out.=		"  </tr>\n";
    }
    ...
    for ($i=0; $i<mysql_num_rows($result); $i++) {
      ...
      $bgcolor =	"";
      if (!$row["active"]) {
        $class='inactive';	$title = '(Reportedly off air or decommissioned)';
      }
      else {
        switch ($row["type"]) {
	...
	."<td><a href='./?mode=signal_list&filter_khz_1=".(float)$row["khz"]."&filter_khz_2=".(float)$row["khz"]."&limit=-1' title='Filter on this value'>".(float)$row["khz"]."</a></td>\n"
	."<td><a onmouseover='window.status=\"View profile for ".(float)$row["khz"]."-".$row["call"]."\";return true;' onmouseout='window.status=\"\";return true;' href='javascript:signal_info(\"".$row["ID"]."\")'><b>$call</b></a></td>\n";
      if ($type_NDB) {
        $out.=
	...
	."<td>".($SP ? "<a href='./?mode=signal_list&filter_sp=".$row["SP"]."' title='Filter on this value'>".$SP."</a>" : "&nbsp;")."</td>\n"
	."<td>".($ITU ? "<a href='./?mode=signal_list&filter_itu=".$row["ITU"]."' title='Filter on this value'>".$ITU."</a>" : "&nbsp;")."</td>\n"
	."<td>".($row["GSQ"] ? "<a onmouseover='window.status=\"View map for ".(float)$row["khz"]."-".$row["call"]."\";return true;' onmouseout='window.status=\"\";return true;' href='javascript:popup_map(\"".$row["ID"]."\",\"".$row["lat"]."\",\"".$row["lon"]."\")' title='Show map (accuracy limited to nearest Grid Square)'><span class='fixed'>".$row["GSQ"]."</span></a>" : "&nbsp;")."</td>\n"
	."<td>".($row["pwr"] ? $row["pwr"] : "&nbsp;")."</td>\n"
	."<td>".($row["notes"] ? stripslashes($row["notes"]) : "&nbsp;")."</td>\n"
	."<td>".($heard_in ? $heard_in : "&nbsp;")."</td>\n"
	."<td align='right'>".($row["logs"] ? "<a onmouseover='window.status=\"View logs for ".(float)$row["khz"]."-".$row["call"]."\";return true;' onmouseout='window.status=\"\";return true;' href='javascript:signal_log(\"".$row["ID"]."\")'><b>".$row["logs"]."</b></a>" : "&nbsp;")."</td>\n"
    ...
    $out.=	"<h1>Results</h1><br><br><h2>No results for search criteria</h2><br><br><br>\n";
  }
  ...
  $out.=
	 "<input type='button' class='formbutton' value='All ".system." > Excel' style='width: 150px' title='Get the whole database as Excel' onclick='if (confirm(\"EXPORT ENTIRE ".system." DATABASE TO EXCEL\\n\\nThis can be a time consuming process - typically well over a minute.\\n\\nSignal types will be listed according to the options you selected in\\n\\\"Customise Report\\\" above, other filtering options are ignored.\\n\\nContinue?\")) { export_signallist_excel(); }'> "
	."<input type='button' class='formbutton' value='All ".system." > PDF' style='width: 150px' title='get the whole database as PDF' onclick='if (confirm(\"EXPORT ENTIRE ".system." DATABASE TO PDF\\n\\nThis can be a time consuming process - typically 5 minutes or more.\\nYou will get much quicker results by viewing all records and printing\\nout directly from your web browser.\\n\\nContinue anyway?\")) { show_pdf(); }'> "
...
function signal_listeners() {
  global $ID, $mode, $submode, $sortBy, $script, $targetID;
...
//  print("<pre>$sql</pre>");
  ...
  $out[] =	"      <tr>\n";
  $out[] =	"        <td colspan='2' width='100%'><table border='0' cellpadding='0' cellspacing='0' width='100%'>\n";
  $out[] =	"          <tr>\n";
  $out[] =	"            <td><h1>Signal Listeners</h1></td>\n";
  ...
  $out[] =	"              <tr>\n"
    ...
    ."            <td bgcolor='#F5F5F5' class='itemTextCell' valign='top'>\n";
  if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
    $out[] =	"            <form action='$script' name='form' method='POST' onsubmit='if (window.opener) { window.opener.location.reload(1)};return true;'>\n";
  ...
  if (mysql_num_rows($result)) {
    $out[] =	"            <table width='100%'  border='0' cellpadding='2' cellspacing='1' class='downloadTable'>\n";
    $out[] =	"              <tr>\n";
    $out[] =	"                <th class='downloadTableHeadings_nosort' align='left'>&nbsp;Logs for ".(float)$khz."-".$call.($QTH ? ", $QTH" : "").($SP ? ", $SP" : "").($ITU ? ", $ITU" : "")."</th>\n";
    $out[] =	"              </tr>\n";
    $out[] =	"              <tr>\n";
    $out[] =	"                <td bgcolor='white'><table cellpadding='0' cellspacing='1' border='0' class='noprint'>\n";
    $out[] =	"                  <thead>\n";
    $out[] =	"                  <tr>\n";
    $out[] =	"                    <th class='scroll_list' width='140' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=name".($sortBy=='name' ? '_d' : '')."'>".($sortBy=='name'||$sortBy=='name_d'?'<font color="#ff0000">Name</font>':'Name')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='140' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=QTH".($sortBy=='QTH' ? '_d' : '')."'>".($sortBy=='QTH'||$sortBy=='QTH_in_d'?'<font color="#ff0000">QTH</font>':'QTH')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=SP".($sortBy=='SP' ? '_d' : '')."'>".($sortBy=='SP'||$sortBy=='SP_d'?'<font color="#ff0000">SP</font>':'SP')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=ITU".($sortBy=='ITU' ? '_d' : '')."'>".($sortBy=='ITU'||$sortBy=='ITU_d'?'<font color="#ff0000">ITU</font>':'ITU')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=".($sortBy=='logs_d' ? 'logs' : 'logs_d')."'>".($sortBy=='logs'||$sortBy=='logs_d'?'<font color="#ff0000">Logs</font>':'Logs')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=".($sortBy=='dx_d' ? 'dx' : 'dx_d')."'>".($sortBy=='dx'||$sortBy=='dx_d'?'<font color="#ff0000">KM</font>':'KM')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=".($sortBy=='dx_d' ? 'dx' : 'dx_d')."'>".($sortBy=='dx'||$sortBy=='dx_d'?'<font color="#ff0000">Miles</font>':'Miles')."</a></small></th>\n";
    $out[] =	"                  </tr>\n";
    $out[] =	"                  </thead>\n";
    $out[] =	"                </table></td>\n";
    $out[] =	"              </tr>\n";
    $out[] =	"              <tr>\n";
    $out[] =	"                <td class='rownormal' bgcolor='#ffffff'>\n";
    $out[] =	"                <div class='scrollbox_230'>\n";
    $out[] =	"                <table cellpadding='0' cellspacing='1' border='0' bgcolor='#ffffff'>\n";
    $out[] =	"                  <thead>\n";
    $out[] =	"                  <tr class='noscreen'>\n";
    $out[] =	"                    <th class='scroll_list'><small>Name</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>QTH</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>SP</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>ITU</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>Logs</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>KM</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>Miles</small></th>\n";
    $out[] =	"                  </tr>\n";
    $out[] =	"                  </thead>\n";
    ...
    for ($i=0; $i<mysql_num_rows($result); $i++) {
      $row =	mysql_fetch_array($result,MYSQL_ASSOC);
      $out[] =	"                  <tr>\n";
      $out[] =	"                    <td class='scroll_list' width='140'><a href=\"javascript:listener_signals('".$row["listenerID"]."')\"><b>".$row["name"]."</b></a></td>";
      $out[] =	"                    <td class='scroll_list' width='140'>".($row["QTH"]   ? $row["QTH"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["SP"]   ? $row["SP"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["ITU"]   ? $row["ITU"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["logs"]   ? $row["logs"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["dx_km"]   ? $row["dx_km"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["dx_miles"]   ? $row["dx_miles"]   : "&nbsp;")."</td>\n";
      $out[] =	"                  </tr>\n";
    }
  $out[] =	"                  </div></table></td>\n";
  $out[] =	"              </tr>\n";
  $out[] =	"              <tr class='noprint'>\n";
  $out[] =	"                <td class='downloadTableContent' align='center'>\n";
...
//  print("<pre>$sql</pre>");
  ...
  if (mysql_num_rows($result)) {
    $out[] =	"            <table width='100%'  border='0' cellpadding='2' cellspacing='1' class='downloadTable'>\n";
    $out[] =	"              <tr>\n";
    $out[] =	"                <th class='downloadTableHeadings_nosort' align='left'><table cellpadding='0' cellspacing='0' border='0' width='100%'><tr><td  class='downloadTableHeadings_nosort'>&nbsp;Logs for ".(float)$khz."-".$call.($QTH ? ", $QTH" : "").($SP ? ", $SP" : "").($ITU ? ", $ITU" : "")."</td><td class='downloadTableHeadings_nosort' align='right'><span style='font-weight: normal'>(daytime logs in <b>bold</b>)</span></td></tr></table></th>\n";
    $out[] =	"              </tr>\n";
    $out[] =	"              <tr>\n";
    $out[] =	"                <td bgcolor='white'><table cellpadding='0' cellspacing='1' border='0' class='noprint'>\n";
    $out[] =	"                  <thead>\n";
    $out[] =	"                  <tr>\n";
    $out[] =	"                    <th class='scroll_list' width='75' title='YYYY-MM-DD' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=date".($sortBy=='date' ? '_d' : '')."'>".($sortBy=='date'||$sortBy=='date_d'?'<font color="#ff0000">Date</font>':'Date')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='35' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=time".($sortBy=='time' ? '_d' : '')."'>".($sortBy=='time'||$sortBy=='time_d'?'<font color="#ff0000">UTC</font>':'UTC')."</a></small></th>\n";
    if (system!="RWW") {
      $out[] =	"                    <th class='scroll_list' width='40' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=LSB".($sortBy=='LSB' ? '_d' : '')."'>".($sortBy=='LSB'||$sortBy=='LSB_d'?'<font color="#ff0000">LSB</font>':'LSB')."</a></small></th>\n";
      $out[] =	"                    <th class='scroll_list' width='40' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=USB".($sortBy=='USB' ? '_d' : '')."'>".($sortBy=='USB'||$sortBy=='USB_d'?'<font color="#ff0000">USB</font>':'USB')."</a></small></th>\n";
      $out[] =	"                    <th class='scroll_list' width='40' align='left'><a href='$script?mode=$mode&ID=$ID&sortBy=sec".($sortBy=='sec' ? '_d' : '')."'>".($sortBy=='sec'||$sortBy=='sec_d'?'<font color="#ff0000">Sec</font>':'Sec')."</a></small></th>\n";
      $out[] =	"                    <th class='scroll_list' width='65' align='left'><a href='$script?mode=$mode&ID=$ID&sortBy=format".($sortBy=='format' ? '_d' : '')."'>".($sortBy=='format'||$sortBy=='format_d'?'<font color="#ff0000">Format</font>':'Format')."</a></small></th>\n";
    }
    $out[] =	"                    <th class='scroll_list' width='140' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=listener".($sortBy=='listener' ? '_d' : '')."'>".($sortBy=='listener'||$sortBy=='listener_d'?'<font color="#ff0000">Listener</font>':'Listener')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='35' align='left'><small><a href='$script?mode=$mode&ID=$ID&sortBy=heard_in".($sortBy=='heard_in' ? '_d' : '')."'>".($sortBy=='heard_in'||$sortBy=='heard_in_d'?'<font color="#ff0000">In</font>':'In')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=dx".($sortBy=='dx' ? '_d' : '')."'>".($sortBy=='dx'||$sortBy=='dx_d'?'<font color="#ff0000">KM</font>':'KM')."</a></small></th>\n";
    $out[] =	"                    <th class='scroll_list' width='40' align='right'><small><a href='$script?mode=$mode&ID=$ID&sortBy=dx".($sortBy=='dx' ? '_d' : '')."'>".($sortBy=='dx'||$sortBy=='dx_d'?'<font color="#ff0000">Miles</font>':'Miles')."</a></small></th>\n";
    if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
      $out[] =	"                    <th class='scroll_list' width='35' align='left'><small>Del</small></th>\n";
    }
    $out[] =	"                  </tr>\n";
    $out[] =	"                  </thead>\n";
    $out[] =	"                </table></td>\n";
    ...
    $out[] =	"                  <thead>\n";
    $out[] =	"                  <tr class='noscreen'>\n";
    $out[] =	"                    <th class='scroll_list'><small>Date</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>UTC</small></th>\n";
    if (system!="RWW") {
      $out[] =	"                    <th class='scroll_list'><small>LSB</small></th>\n";
      $out[] =	"                    <th class='scroll_list'><small>USB</small></th>\n";
      $out[] =	"                    <th class='scroll_list'><small>Sec</small></th>\n";
      $out[] =	"                    <th class='scroll_list'><small>Format</small></th>\n";
    }
    $out[] =	"                    <th class='scroll_list'><small>Listener</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>In</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>KM</small></th>\n";
    $out[] =	"                    <th class='scroll_list'><small>Mi</small></th>\n";
    if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
      $out[] =	"                    <th class='scroll_list'><small>Del</small></th>\n";
    }
    $out[] =	"                  </tr>\n";
    $out[] =	"                  </thead>\n";
    ...
    for ($i=0; $i<mysql_num_rows($result); $i++) {
      $row =	mysql_fetch_array($result,MYSQL_ASSOC);
      $out[] =	"                  <tr>\n";
      $out[] =	"                    <td class='scroll_list' width='75'>".($row["date"] ? $row["date"] : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='35'>".($row["time"] ? ($row['daytime'] ? "<b>".$row["time"]."</b>" : $row["time"]) : "&nbsp;")."</td>\n";
      if (system!="RWW") {
        $out[] =	"                    <td class='scroll_list' width='40'>".$row["LSB_approx"].($row["LSB"]<>""  ? $row["LSB"]  : "&nbsp;")."</td>\n";
        $out[] =	"                    <td class='scroll_list' width='40'>".$row["USB_approx"].($row["USB"]<>""  ? $row["USB"]  : "&nbsp;")."</td>\n";
        $out[] =	"                    <td class='scroll_list' width='40'>".($row["sec"]  ? $row["sec"]  : "&nbsp;")."</td>\n";
        $out[] =	"                    <td class='scroll_list' width='65'>".($row["format"]  ? $row["format"]  : "&nbsp;")."</td>\n";
      }
      $out[] =	"                    <td class='scroll_list' width='140'><a href=\"javascript:listener_log('".$row["listenerID"]."')\"><b>".$row["name"]."</b></a></td>";
      $out[] =	"                    <td class='scroll_list' width='35'>".($row["heard_in"]   ? $row["heard_in"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["dx_km"]   ? $row["dx_km"]   : "&nbsp;")."</td>\n";
      $out[] =	"                    <td class='scroll_list' width='40' align='right'>".($row["dx_miles"]   ? $row["dx_miles"]   : "&nbsp;")."</td>\n";
      if (isset($_COOKIE['cookie_admin']) && $_COOKIE['cookie_admin']==admin_session) {
        $out[] ="                    <td class='scroll_list' width='35'><a href='$script?mode=$mode&submode=delete&ID=$ID&targetID=".$row["ID"]."'>Del</a></td>\n";
      }
      $out[] =	"                  </tr>\n";
    }
  $out[] =	"                  </div></table></td>\n";
...
function signal_map_eu() {
  ...
  $out[] =
    ...
    ." src=\"./?mode=generate_station_map&amp;ID=$ID&amp;system_ID=2\"\n"
    ." border=\"0\" alt=\"\" />\n"
    ."<map name=\"map\">\n";
  ...
  $out[] =
     "</map>\n"
    ."<div ID='point_here' style='position: absolute; display: none;'><img alt=\"\" src='../assets/map_point_here.gif' /></div>\n"
    ."<div ID='name_here' style='position: absolute; display: none;'><img alt=\"\" src='../assets/map_name_here.gif' /></div>\n";
  return implode($out,"");
}
...
function signal_map_na() {
  global $ID;
  $out =	array();
  $out[] =	"<img usemap=\"#map\" galleryimg=\"no\" src=\"./?mode=generate_station_map&ID=$ID&system_ID=1\" border=\"0\">\n";
  $out[] =	"<map name=\"map\">\n";
...
//print("<pre>$sql</pre>");
  ...
  $out[] =	 "</map>\n"
		...
		."  document.write(\"<layer name='name_here'><img src='../assets/map_name_here.gif'></layer>\");\n"
		."}\n"
		."else {\n"
		."  document.write(\"<div ID='point_here' style='position: absolute; display: none;'><img src='../assets/map_point_here.gif'></div>\");\n"
		."  document.write(\"<div ID='name_here' style='position: absolute; display: none;'><img src='../assets/map_name_here.gif'></div>\");\n"
		."}\n"
		."</script>\n";
...
function signal_merge() {
  ...
  $result =	mysql_query($sql);
  $row =	mysql_fetch_array($result, MYSQL_ASSOC);
  $out[] =	"<form action='$script?mode=$mode&submode=merge&ID=$ID' method='POST'>\n";
...
// ************************************
function signal_QNH() {
  global $ID, $mode, $script, $submode, $ICAO_signal, $hours_signal;
  ...
  if ($ICAO_signal && $hours_signal) {
    if ($METAR = METAR($ICAO_signal,$hours_signal,0)) {
      ...
      $pressure[] =	"QNH at $ICAO_signal -\n".$row["name"].($row["SP"] ? ", ".$row["SP"] : "").", ".$row["CNT"];
      $pressure[] =	"(".$dx[0]." miles from $khz-$call)\n";
      $pressure[] =	"----------------------\nDD UTC  MB     SLP \n----------------------";
      $pressure[] =	$METAR;
      $pressure[] =	"----------------------\n(From ".system.")\n";
    }
  ...
  $out[] =	"                  <tr>\n";
  $out[] =	"                    <th align='left' class='downloadTableHeadings_nosort'>&nbsp;QNH History for ".(float)$khz."-".$call.($QTH ? ", $QTH" : "").($SP ? ", $SP" : "").($ITU ? ", $ITU" : "")."</th>\n";
  $out[] =	"                  </tr>\n";
  $out[] =	"                </table></th>\n";
  $out[] =	"              </tr>\n";

  $out[] =	"              <tr>\n";
  $out[] =	"                    <td class='downloadTableContent' width='100%'><select class='formFixed' name='ICAO_signal'>\n";

  $icao_arr =	get_local_icao($GSQ,10,$ICAO_signal);
  for ($i=0; $i<10; $i++) {
    $out[] =	"<option value='".$icao_arr[$i]['ICAO']."'".($ICAO_signal==$icao_arr[$i]['ICAO'] ? " selected" : "").">".$icao_arr[$i]['ICAO']." (".$icao_arr[$i]['miles']." Miles, ".$icao_arr[$i]['km']." KM)</option>\n";
  }
  $out[] =	"</select>\n";
  $out[] =	"<input type='text' size='2' maxlength='2' name='hours_signal' value='".($hours_signal ? $hours_signal : "12")."' class='formField'> Hours\n";
  $out[] =	"<input type='submit' value='QNH' class='formButton'>\n";
  $out[] =	"<input type='button' value='METAR' class='formButton' onclick='popWin(\"http://adds.aviationweather.noaa.gov/metars/index.php?station_ids=\"+document.pressure.ICAO_signal.value+\"&std_trans=&chk_metars=on&hoursStr=past+\"+document.pressure.hours_signal.value+\"+hours\",\"popMETAR\",\"scrollbars=1,resizable=1,location=1\",640,380,\"\");'>\n";
  $out[] =	"<input type='button' value='Decoded' class='formButton' onclick='popWin(\"http://adds.aviationweather.noaa.gov/metars/index.php?station_ids=\"+document.pressure.ICAO_signal.value+\"&std_trans=translated&chk_metars=on&hoursStr=past+\"+document.pressure.hours_signal.value+\"+hours\",\"popDecoded\",\"scrollbars=1,resizable=1,location=1\",640,380,\"\");'></td>\n";
  $out[] =	"                  </tr>\n";
  $out[] =	"                  <tr height='100%'>\n";
  $out[] =	"                    <td class='downloadTableContent' width='100%'><textarea rows='10' cols='60' style='width: 580px; height: 95%;' class='formFixed'>".implode($pressure,"\n")."</textarea></td>\n";
  $out[] =	"                  </tr>\n";
  $out[] =	"            </table></form></td>\n";
  $out[] =	"          </tr>\n";
  $out[] =	"        </table></td>\n";
...
function signal_seeklist() {
  global $script, $mode, $submode, $paper, $createFor, $region, $targetID, $filter_active, $filter_date_1, $filter_date_2;
...
//        print "<pre>$sql</pre>";
  ...
  print		 "<form name='form' action='$script' method='POST'>\n"
		...
		."<input type='hidden' name='submode' value=''>\n"
		."<input type='hidden' name='targetID' value=''>\n"
		."<h1><a name='top'></a>Signal Seeklist</h1>\n"
		."<table cellpadding='2' border='0' cellspacing='1'>\n"
		."  <tr>\n"
		."    <td align='center' valign='top' colspan='2'><table cellpadding='0' border='0' cellspacing='0' width='100%'>\n"
		."      <tr>\n"
  ...
  print		 "<table cellpadding='2' cellspacing='1' border='0' style='page-break-after: always;' valign='top' class='downloadtable'>\n"
		."  <tr>\n"
  ...
  foreach($signals as $key=>$value) {
    if ($value['SP'] != $SP || $value['ITU'] != $ITU) {
      ...
      print	"    <table cellpadding='0' cellspacing='0' border='0' width='100%'><tr><td><b>".$value['ITU']." ".$value['SP']."</b></td><td align='right'>(".($createFor!="" ? $itu_sp[$value['ITU']."_".$value['SP']]['heard']." of " : "").$itu_sp[$value['ITU']."_".$value['SP']]['total'].")</td></tr></table>\n";
      $xpos+=	$heading_height;
      $SP =	$value['SP'];
      $ITU =	$value['ITU'];
    }
    ...
    print	 "<span class='fixed'$bgcolor>".pad_dot($value["khz"],8).pad_dot($value['call'],8)
		.($value['heard'] ? "<img src='../assets/icon-tick-on.gif'>" : "<img src='../assets/icon-tick-off.gif'>")."<br></span>\n";
    $xpos+=	$line_height;
    if ($xpos>$row) {
      $xpos=0;
      $col++;
      print	"</td>\n";
      if ($col<$page_cols) {
        print		"<td valign='top' nowrap width='".(100/$page_cols)."%' class='downloadTableContent'>";
      }
      else {
        print		 "</td>\n"
			."</tr>\n"
  ...
  if ($col<$page_cols) {
    ...
    for ($col;$col<$page_cols-1; $col++) {
      print	"<td valign='top' nowrap width='".(100/$page_cols)."%' class='downloadTableContent'><span class='fixed'>&nbsp;</span></td>\n";
    }
  }
