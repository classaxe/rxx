...
signal.markerClickFunction = function(s, latlng) {
    return function(e) {
        ...
        var infoHtml =
            ...
            '    <label>Type</label>'+s.type + '<br />' +
            (s.pwr!='0' ? '    <label>Power</label>'+s.pwr + 'W<br />' : '') +
            '    <label>\'Name\' / QTH</label>'+s.qth + (s.sp ? ', ' + s.sp : '') + ', ' + s.itu + '<br />' +
            (s.gsq ? '    <label>GSQ</label><a href="." onclick="popup_map('+s.id+','+s.lat+','+s.lon+');return false;" title="Show map (accuracy limited to nearest Grid Square)">'+s.gsq+'</a><br />' : '') +
