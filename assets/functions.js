...
Version History:
  ...
  1.0.3 (2015-01-12)
    1) Moved line_up() and pad() into here
  1.0.2 (2014-03-19)
    1) Moved some inline JS functions into here and sorted alphabetically
  1.0.1 (2013-09-24)
    1) Added support for type DSC in clear_signal_list()
*/
...
function find_ICAO() {
  popWin(system_URL+'/find_ICAO','popFindICAO','scrollbars=0,toolbar=0,menubar=0,status=0,resizable=1',250,180,'centre');
}
...
function listener_log(ID){
  popWin(system_URL+'/listener_log/'+ID,'popListener','scrollbars=0,resizable=1',640,380,'centre');
}

function listener_map(system_ID){
  switch (system_ID){
    case 1:
      popWin(system_URL+'/listener_map/'+system_ID,'popListenerMap1','scrollbars=1,resizable=1',960,660,'centre');
    break;
    case 2:
      popWin(system_URL+'/listener_map/'+system_ID,'popListenerMap2','scrollbars=1,resizable=1',1000,700,'centre');
    break;
  }
}

function listener_signals(ID){
  popWin(system_URL+'/listener_signals/'+ID,'popListener','scrollbars=0,resizable=1',640,380,'centre');
}
...
function poll_edit(ID){
  popWin(system_URL+'/poll_edit?ID='+ID,'pollEdit','scrollbars=1,resizable=1',600,340,'centre');
}
...
function show_itu(){
  popWin(system_URL+'/show_itu'+(arguments.length ? '?region=' + arguments[0] : ''),'popITU','scrollbars=1,toolbar=0,menubar=0,status=0,resizable=1',560,550,'centre');
}
...
function show_sp(){
  popWin(system_URL+'/show_sp','popSP','scrollbars=1,toolbar=0,menubar=0,status=0,resizable=1',560,550,'centre');
}
...
function signal_add(call,khz,GSQ,QTH,SP,ITU,PWR,type){
  popWin(system_URL+'/signal_info?call='+call.replace(/#/,"%23")+'&khz='+khz+'&GSQ='+GSQ+'&QTH='+QTH+'&SP='+SP+'&ITU='+ITU+'&pwr='+PWR+'&type='+type,'popsignalAdd','scrollbars=0,resizable=1,status=1',640,380,'centre');
}
...
function signal_log(ID){
  popWin(system_URL+'/signal_log/'+ID,'popsignal','scrollbars=0,resizable=1',640,380,'centre');
}
...
function signal_merge(ID){
  popWin(system_URL+'/signal_merge?ID='+ID,'popsignalMove','scrollbars=0,resizable=1,status=1',640,380,'centre');
}
