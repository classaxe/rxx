...
// ************************************
function generate_station_map() {
  global $ID,$system_ID;

  switch ($system_ID) {
    case "1":	$region= "(`listeners`.`region`='na' OR `listeners`.`region`='ca' OR `listeners`.`itu` = 'HWA')";	break;
    case "2":	$region="`listeners`.`region`='eu'";	break;
  }
  ...
  $sql =	"SELECT `call`,`heard_in`,`itu`,`khz`,`sp` FROM `signals` WHERE `ID` = \"$ID\"";
  $result =	mysql_query($sql);
  ...
  $sql =	"SELECT `map_x`,`map_y`,`primary_QTH` FROM `listeners` WHERE $region";
  $result =	@mysql_query($sql);
  $reporters =	array();
  for ($i=0; $i<mysql_num_rows($result); $i++) {
    $row =	mysql_fetch_array($result,MYSQL_ASSOC);
...
//  print "<pre>$sql</pre>";
  ...
  for ($i=0; $i<mysql_num_rows($result); $i++) {
    ...
    if ($row['map_x']) {
      ...
      $reporter_rxed[] =	$row['map_x'].",".$row['map_y'].",".$row['daytime'].",".$row['primary_QTH'].",".pad($row['heard_in'],3)." ".pad($name,20).lead($row['dx_miles'],4);
    }
  }
  ...
  switch($system_ID) {
    case 1:
      draw_station_map_na($based_in,$reporters_in,$reporters,$heard_in,$reporter_rxed,$text);
    break;
    case 2:
      draw_station_map_eu($based_in,$reporters_in,$reporters,$heard_in,$reporter_rxed,$text);
    break;
  }
}
...
// ************************************
function generate_listener_map() {
  global $ID,$system_ID;
  ...
  for ($i=0; $i<mysql_num_rows($result); $i++) {
    ...
    if ($row['map_x']) {
      ...
      $reporter_rxed[] =	$row['map_x'].",".$row['map_y'].",,".$row['primary_QTH'].",".pad($row['heard_in'],3)." ".pad($name,20);
    }
  }

  switch($system_ID) {
    case 1:
      draw_station_map_na("","",$reporters,$reporters_in,"","");
    break;
    case 2:
      draw_station_map_eu("","",$reporters,$reporters_in,"","");
    break;
  }
}
...
function generate_map_eu(){
  ...
  draw_fill_country_list_eu("ENG,POR,DEU,ALB,HNG,TUR,LVA,ISL",$image,$map_1);
  draw_fill_country_list_eu("IRL,ESP,SUI,SVK,SMR,NOR",$image,$map_2);
  draw_fill_country_list_eu("WLS,GSY,AND,ITA,CZE,SER,MDA,RUS",$image,$map_3);
  draw_fill_country_list_eu("IOM,FRA,DNK,SVN,CVA,BUL,LTU",$image,$map_4);
  draw_fill_country_list_eu("SHE,JSY,BEL,COR,AUT,BIH,KAL,GRC,UKR,SWE",$image,$map_5);
  draw_fill_country_list_eu("SCT,LUX,SAR,HRV,MKD,BLR,GEO,FIN",$image,$map_6);
  draw_fill_country_list_eu("GSY,ORK,NIR,HOL,BAL,SCY,LIE,POL,MON,ROU,EST,FRO",$image,$map_7);

  ImageRectangleWithRoundedCorners($image,3,5,300,56,5,$darkgray,$white);
...
function generate_map_na(){
  ...
  draw_fill_country_list_na("CA,WA,TX,NM,VA,NE,NC,SC,MN,IL,FL,AZ,MO,OR,MI,CO,IN,HI,AK,AL,MS,PA,NY,TN,MD,WV,ID,NV,UT,MT,WY,ND,SD,KS,OK,AR,LA,GA,IA,KY,OH,DC,DE,NJ,RI,CT,MA,VT,NH,ME,WI,PTR",$image,$map_1);
  draw_fill_country_list_na("YT,BC,NT,AB,SK,NU,MB,ON,QC,NL,NB,PE,NS",$image,$map_2);
  draw_fill_country_list_na("GRL,MEX,JMC,SVD,PNR",$image,$map_3);
  draw_fill_country_list_na("BAH,GTM,DOM",$image,$map_4);
  draw_fill_country_list_na("BLZ,CTR,BER,HTI,VIR",$image,$map_5);
  draw_fill_country_list_na("CUB,NCG,VRG",$image,$map_6);
  draw_fill_country_list_na("HND,PTR",$image,$map_7);

  ImageRectangleWithRoundedCorners($image,3,565,300,616,5,$darkgray,$white);
...
// * draw_station_map_eu()              *
// **************************************
function draw_station_map_eu($based_in,$reporters_in,$reporters,$heard_in,$reporter_rxed,$text){
  global $map;
  ...
  if (isset($heard_in)) {
    draw_fill_country_eu("sea",$image,$map_sea);
    draw_fill_country_list_eu($reporters_in,$image,$col_heard_no);
  }
  if (isset($heard_in)) {
    draw_fill_country_list_eu(strtoupper($heard_in),$image,$col_heard_yes);
  }
  if (isset($based_in)) {
    draw_fill_country_list_eu(strtoupper($based_in),$image,$blue);
  }
  if (isset($text)) {
    draw_map_legend($image,$text,$blue,$col_heard_yes,$col_heard_no,3,3,$img_point1,$img_point2,$img_point3,$img_point4);
  }
  if (isset($reporter_o)) {
  ...
  if (isset($reporters)) {
    draw_map_point_list($reporters,$image,$img_point3,$img_point4);
  }

  if (isset($reporter_rxed)) {
    draw_map_point_list_names($reporter_rxed,$image,$img_point1,$img_point2,695,0);
  }
...
// * draw_station_map_na()              *
// **************************************
function draw_station_map_na($based_in,$reporters_in,$reporters,$heard_in,$reporter_rxed,$text){
  $image =		imageCreate(653,620);
  $image_map =		imageCreateFromGif("../assets/na_map_outline.gif");
  ...
  if ($heard_in) {
    draw_fill_country_na("sea",$image,$map_sea);
    draw_fill_country_list_na($reporters_in,$image,$col_heard_no);
  }
  if ($heard_in) {
    draw_fill_country_list_na(strtoupper($heard_in),$image,$col_heard_yes);
  }
  if ($based_in) {
    draw_fill_country_list_na(strtoupper($based_in),$image,$blue);
  }
  if ($text) {
    draw_map_legend($image,$text,$blue,$col_heard_yes,$col_heard_no,520,535,$img_point1,$img_point2,$img_point3,$img_point4);
  }
  if ($reporters) {
    draw_map_point_list($reporters,$image,$img_point3,$img_point4);
  }
  ...
  if ($reporter_rxed) {
    draw_map_point_list_names($reporter_rxed,$image,$img_point1,$img_point2,3,578-(count(explode("|",$reporter_rxed))*8));
  }
...
// **************************************
// * draw_fill_country_list_eu()        *
// **************************************
function draw_fill_country_list_eu($list,&$image,$color) {
  $arr = explode(",",$list);
  for ($i=0; $i<count($arr); $i++) {
    draw_fill_country_eu($arr[$i],$image,$color);
  }
}
...
// **************************************
// * draw_fill_country_list_na()        *
// **************************************
function draw_fill_country_list_na($list,&$image,$color) {
  $arr = explode(",",$list);
  for ($i=0; $i<count($arr); $i++) {
    draw_fill_country_na($arr[$i],$image,$color);
  }
}
...
// **************************************
// * draw_fill_country_eu()             *
// **************************************
function draw_fill_country_eu($country,&$image,$color) {
  switch ($country) {
    case "AND":
...
// **************************************
// * draw_fill_country_na()             *
// **************************************
function draw_fill_country_na($country,&$image,$color) {
  switch ($country) {
    case "AB":
...
// * draw_map_legend()                *
// ************************************
function draw_map_legend(&$image,$text,$col_tx,$col_heard,$col_rep,$x,$y,&$img_point1,&$img_point2,&$img_point3,&$img_point4) {
  $black =	ImageColorAllocate($image,0,0,0);
  $white =	ImageColorAllocate($image,255,255,255);
  $blue =	ImageColorAllocate($image,120,120,255);
  $gray =	ImageColorAllocate($image,150,150,150);
  ...
  ImageRectangleWithRoundedCorners($image,$x+3,$y+3,$x+129,$y+81,5,$darkgray,$darkgray);
  ImageRectangleWithRoundedCorners($image,$x,$y,$x+126,$y+78,5,$black,$white);
...
// **************************************
// * ImageRectangleWithRoundedCorners() *
// **************************************
function ImageRectangleWithRoundedCorners(&$im, $x1, $y1, $x2, $y2, $radius, $linecolor, $fillcolor=false) {
  if ($fillcolor) {
...
// * draw_map_point_list()            *
// ************************************
function draw_map_point_list($list,&$image,&$img_point3,&$img_point4) {
  $arr = explode("|",$list);
  for ($i=0; $i<count($arr); $i++) {
...
// ************************************
// * draw_map_point_list_names()      *
// ************************************
function draw_map_point_list_names($list,&$image,&$img_point,&$img_point2,$x,$y) {
  ...
  ImageRectangleWithRoundedCorners($image,$x+3,$y+3,$x+162,(count($arr)*8)+$y+3+35,5,$darkgray,$darkgray);
  ImageRectangleWithRoundedCorners($image,$x,$y,$x+159,(count($arr)*8)+$y+35,5,$black,$white);
  ...
  for ($i=0; $i<count($arr); $i++) {
    ...
    if ($daytime) {
      ImageString($image,1,$x+13,($i*8)+$y+20,stripslashes($name),$black);
    }
    else {
      ImageString($image,1,$x+13,($i*8)+$y+20,stripslashes($name),$darkgray);
    }
  }
