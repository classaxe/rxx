...
switch(cookie['language']) {
  case "French":	months = new Array('Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'); break;
  case "German":	months = new Array('Januar','Februar','März','April','Mag','Juni','Juli','August','September','Oktober','November','Dezember'); break;
  case "Italian":	months = new Array('Gennaio','Febbraio','Marzo','Aprile','Può','Giugno','Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'); break;
  case "Portuguese":months = new Array('Janeiro','Fevereiro','Março','Abril','Pode','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'); break;
  case "Spanish":	months = new Array('Enero','Febrero','Marcha','Abril','Puede','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'); break;
  default:		months = new Array('January','February','March','April','May','June','July','August','September','October','November','December'); break
}
...
function LOG(khz,call,yyyymmdd,hhmm,notes) {
  ...
  if (!stats.year[yyyy]) {
    ...
    stats.year[yyyy].new_beacons =	0;			// Count new Beacons
    stats.year[yyyy].new_beacons_ever =	0;		// Count new Beacons
  ...
  if (!stats.year[yyyy][mm]) {
    ...
    stats.year[yyyy][mm].new_beacons =		0;		// Count new beacons
    stats.year[yyyy][mm].new_beacons_ever =	0;		// Count new beacons
  ...
  if (!station[id].all_date) {
    ...
    if (!stats.all[id]) {
      stats.year[yyyy][mm].new_beacons_ever++
      stats.year[yyyy].new_beacons_ever++
  ...
  if (!stats.year[yyyy].id[id]) {
      stats.year[yyyy].new_beacons++;			// and add it to the new beacons count
      stats.year[yyyy][mm].new_beacons++;		// and add it to the new beacons count, and add it to this month's total also.
  }
...
// ************************************
// Used in text output
function TEXT(yyyymmddhhmm,date,hhmm,khz,call,gsq,lsb,usb,cyc,pwr,dx,dxw,nu,qth,sta,cnt) {
...
function list(yyyy,mm) {
  ...
  out[n++] =	"<head><link TITLE='new' REL='stylesheet' HREF='ndbweblog.css' type='text/css'>\n"+
			...
			"<a href='javascript:void top.popup_map(top.qth_lat,top.qth_lon)'"+msg_m+" title='Click to see a map of this location'>Location</a> "+qth_lat+", "+qth_lon+"</b><br>\n"+
			"<small>NDB WebLog Software: "+version+" by <a href='mailto:martin@classaxe.com' title='Bug reports? Suggestions? Contact the programmer'><b>Martin Francis</b></a>, &copy; 2003, 2004<br>\n"+
			"This <a href='http://www.classaxe.com/dx' target='_blank'"+msg_dl+" title='NDB WebLog is free - get it here!'><b>NDB WebLog</b></a> is configured as a "+
			"<b>"+((monthly)?("Monthly"):("Lifetime"))+" list</b>, "+
			"all <b>times</b> are in <a href='http://www.timeanddate.com/worldclock' target='_blank'"+msg_utc+" title='View world clock'><b>UTC</b></a>, "+
			"and the <b>last log entry</b> was recorded on <b>"+format_date(stats.last_date)+" at "+stats.last_time+" UTC</b><br>";
  ...
  // Table Headings:
  out[n++] =	"<tr>\n"+
			"<th ROWSPAN="+rows+(sel_sort=="khz"||sel_sort=="khz_d"   ? " class='khz_sort" : " class='khz'")+(sel_sort=="khz_d" ? "_d" : "")+"'>"+
			...
			"<th ROWSPAN="+rows+(sel_sort=="cnt" ||sel_sort=="cnt_d"  ? " class='sort" : "")+(sel_sort=="cnt_d"  ? "_d" : "")+"'>"+
  ...
  out[n++] =	"<th ROWSPAN="+rows+(sel_sort=="pwr"  ||sel_sort=="pwr_d" ? " class='sort" : "")+(sel_sort=="pwr_d"  ? "_d" : "")+"'>"+
			...
			"<a "+href+"dx"+  (sel_sort=="dx"   ? "_d" : "")+"\")'"+(sel_sort=="dx"   ? msg_d : msg)+" title='Distance to beacon from reception location'>"+
			get_units()+"</a></th>\n";
  ...
  //   click reverses sort order in month
    ...
    out[n++] =	"<th COLSPAN=2>Night<br>"+lead((utc_daylight+4)%24)+":00-<br>"+lead((utc_daylight-1)%24)+":59</th>\n<th COLSPAN=2>Day<br>"+lead(utc_daylight)+":00-<br>"+lead((utc_daylight+3)%24)+":59</th>\n</TR>\n\n"+
			"<TR>\n<TH"+((sel_sort=="ndd")?(" class='sort'"):(""))+((sel_sort=="ndd_d")?(" class='sort_d'"):(""))+"><a "+href+"ndd"+((sel_sort=="ndd")?("_d"):(""))+"\")'"+((sel_sort=="ndd")?(msg_d):(msg))+" title='Day of Month'>D</th>\n"+
			"<TH"+((sel_sort=="nhhmm")?(" class='sort'"):(""))+((sel_sort=="nhhmm_d")?(" class='sort_d'"):(""))+"><a "+href+"nhhmm"+((sel_sort=="nhhmm")?("_d"):(""))+"\")'"+((sel_sort=="nhhmm")?(msg_d):(msg))+" title='Time (UTC)'>UTC</th>\n"+
			"<TH"+((sel_sort=="ddd")?(" class='sort'"):(""))+((sel_sort=="ddd_d")?(" class='sort_d'"):(""))+"><a "+href+"ddd"+((sel_sort=="ddd")?("_d"):(""))+"\")'"+((sel_sort=="ddd")?(msg_d):(msg))+" title='Day of Month'>D</th>\n"+
			"<TH"+((sel_sort=="dhhmm")?(" class='sort'"):(""))+((sel_sort=="dhhmm_d")?(" class='sort_d'"):(""))+"><a "+href+"dhhmm"+((sel_sort=="dhhmm")?("_d"):(""))+"\")'"+((sel_sort=="dhhmm")?(msg_d):(msg))+" title='Time (UTC)'>UTC</th>\n";
  }

  if (cookie['h_lifelist']!='1') {
    out[n++] =	"<th "+((sel_sort=="all_date")?(" class='sort'"):(""))+((sel_sort=="all_date_d")?(" class='sort_d'"):(""))+"><a "+href+"all_date"+((sel_sort=="all_date")?("_d"):(""))+"\")'"+((sel_sort=="all_date")?(msg_d):(msg))+" title='Date (in format shown in column)'>"+show_date_heading()+"</th>\n<th "+((sel_sort=="all_time")?(" class='sort'"):(""))+((sel_sort=="all_time_d")?(" class='sort_d'"):(""))+"><a "+href+"all_time"+((sel_sort=="all_time")?("_d"):(""))+"\")'"+((sel_sort=="all_time")?(msg_d):(msg))+" title='Time (UTC)'>UTC</th>\n";
  }
  ...
  if (sel_sort) {
    switch(sel_sort) {
      ...
      case "01_d": case "02_d": case "03_d": case "04_d": case "05_d": case "06_d": case "07_d": case "08_d": case "09_d": case "10_d": case "11_d": case "12_d":
    ...
    switch(sel_sort) {
      case "khz":sorted.sort(sortBy_khz);break;
      case "call":sorted.sort(sortBy_call);break;
      case "qth":sorted.sort(sortBy_qth);break;
      case "sta":sorted.sort(sortBy_cnt).sort(sortBy_sta); break;
      case "cnt":sorted.sort(sortBy_sta).sort(sortBy_cnt);break;
      case "01":case "02":case "03":case "04":case "05":case "06":case "07":case "08":case "09":case "10":case "11":case "12":sorted.sort(sortBy_mm);break;
      case "01_d":case "02_d":case "03_d":case "04_d":case "05_d":case "06_d":case "07_d":case "08_d":case "09_d":case "10_d":case "11_d":case "12_d":sorted.sort(sortBy_mm_d);break;
      case "ddd":case "dhhmm":case "ndd":case "nhhmm":sorted.sort(sortBy_temp);break;
      case "ddd_d":case "dhhmm_d":case "ndd_d":case "nhhmm_d":sorted.sort(sortBy_temp_d);break;
...
//  fnTimerStart("List_0");	// Benchmark timer function
  ...
  for (var a=0; a<sorted.length; a++) {
    total_ever++;
    out[n++] =	"<tr>\n<td class='khz'><a name='"+	sorted[a].id+"'></a>"+sorted[a].khz+"</td>\n<td class='call'><a title='Display details for this beacon' href='javascript:void top.popup_details(\""+	sorted[a].id+"\")'>"+sorted[a].call+"</a></td>\n<td class='qth'>"+(sorted[a].lat ? "<a href='javascript:top.popup_map(\""+sorted[a].lat+"\",\""+sorted[a].lon+"\")'"+msg_m+" title='Click to show a map of this location'>"+sorted[a].qth+"</a>" : (sorted[a].qth ? sorted[a].qth : "&nbsp;"))+"</td>\n<td class='sta'>"+(sorted[a].sta ? "<a class='info' href='javascript:void 0' "+status_msg("State = "+sta_arr[sorted[a].cnt][sorted[a].sta]) + " title='"+sta_arr[sorted[a].cnt][sorted[a].sta]+"'>"+sorted[a].sta+"</a>" : "&nbsp;")+"</td>\n<td class='cnt'>"+(sorted[a].cnt ? "<a class='info' href='javascript:void 0' "+status_msg("Country = "+cnt_arr[sorted[a].cnt].name) + " title='"+cnt_arr[sorted[a].cnt].name+"'>"+sorted[a].cnt+"</a>" : "&nbsp;")+"</td>\n";
    if (cookie['h_gsq']!='1') {
  ...
  out[n++] =	"</table>\n";
  out[n++] =	"<b><a name='bottom'></a>"+total_ever+" beacons received";
  if (monthly) {
    out[n++] =	" with " + total + " in "+sel_yyyy+(top.stats.year[sel_yyyy] ? " (including " + top.stats.year[sel_yyyy]['new_beacons_ever'] + " new)" : "")+" and "+total_month+" in " + months[sel_mm-1] + " " + sel_yyyy+	(top.stats.year[sel_yyyy] && top.stats.year[sel_yyyy][sel_mm] && top.stats.year[sel_yyyy][sel_mm]['new_beacons_ever'] ? " (including " + top.stats.year[sel_yyyy][sel_mm]['new_beacons_ever'] + " new)" : "");
  }
  out[n++] =	".</b><br><br>\n<script language='javascript' type='text/javascript'>if(top.show_time) { top.show_time()}</script></body></html>"
//  fnTimerEnd("List_0",0);
...
function popup_details(id) {
  ...
  var map=	((station[id].lat + station[id].lon !="")?
			 ...
			 ("&nbsp;"));
  out[n++] =	"<html><title>NDB WebLog > Details > "+id+"</title>\n<link TITLE='new' REL='stylesheet' HREF='ndbweblog.css' type='text/css'>\n<script language='javascript' type='text/javascript'>\n";
  ...
  if (logbook[id]) {
    out[n++] =	"<h3>Reception Details for "+id+"</h3><table border='"+((document.all)?("0"):("1"))+"' cellspacing='0' cellpadding='1' border='0'>\n";
...
function popup_prefs() {
  ...
  out[n++] =	"  var h_logs =	form.h_logs.options[form.h_logs.selectedIndex].value;\n";
  out[n++] =	"  var lang =		form.lang.options[form.lang.selectedIndex].value;\n";
  ...
  out[n++] =	"                h_notes+D+map_zoom+D+units+D+mod_abs+D+h_logs+D+lang+\";expires=\"+expires;\n";
  ...
  out[n++] =	"  <tr>\n";
  out[n++] =	"    <td class='plain'><table cellpadding='1' cellspacing='0' border='0' class='r' width='100%'>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'>Preferences > Options</th>";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Dates&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Map Zoom&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Distances&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Language&nbsp;</td>\n";
  out[n++] =	"        <td><select name='lang'>\n";
  out[n++] =	"    <option value='English'"+((cookie['language']=='English')?(" selected"):(""))+">English</option>\n"
  out[n++] =	"    <option value='French'"+((cookie['language']=='French')?(" selected"):(""))+">French</option>\n"
  out[n++] =	"    <option value='German'"+((cookie['language']=='German')?(" selected"):(""))+">German</option>\n"
  out[n++] =	"    <option value='Portuguese'"+((cookie['language']=='Portuguese')?(" selected"):(""))+">Portuguese</option>\n"
  out[n++] =	"    <option value='Spanish'"+((cookie['language']=='Spanish')?(" selected"):(""))+">Spanish</option>\n"
  out[n++] =	"    </select>\n";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Mod Values&nbsp;</td>\n";
  ...
  out[n++] =	"    <td class='plain'><table cellpadding='1' cellspacing='0' border='0' class='r' width='100%'>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'>Preferences > Columns</th>";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Morse / DGPS ID&nbsp;</td>";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>DX per Watt&nbsp;</td>";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>First Received&nbsp;</td>";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Log Count</td>";
  out[n++] =	"        <td><select name='h_logs'>";
  out[n++] =	"    <option value='0'"+((cookie['h_logs']=='1')?(""):(" selected"))+">Show</option>\n"
  out[n++] =	"    <option value='1'"+((cookie['h_logs']=='1')?(" selected"):(""))+">Hide</option>\n"
  out[n++] =	"    </select>\n";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td class='l_edge'>Notes&nbsp;</td>";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'><input type='button' value='Submit' onclick='set_cookies(document.form);window.opener.location.reload(1);window.close()'>";
...
function popup_search() {
  ...
  out[n++] =	"<table cellpadding='1' cellspacing='0' border='0' class='r' align='center'>\n";
  out[n++] =	"  <tr>\n";
  out[n++] =	"    <th colspan='2' class='l_edge'>Search</th>";
  out[n++] =	"  </tr>\n";
  out[n++] =	"  <tr>\n";
  out[n++] =	"    <td nowrap class='l_edge'>KHz / Call&nbsp;</td>";
  out[n++] =	"    <td><input name='term'>\n";
  out[n++] =	"  </tr>\n";
  out[n++] =	"  <tr>\n";
  out[n++] =	"    <th colspan='2' class='l_edge'><input type='button' value='Submit' onclick='window.opener.search(document.form);'>";
  out[n++] =	"     <input type='button' value='Cancel' onclick='window.close()'></th>";
  out[n++] =	"  </tr>\n";
  out[n++] =	"</table>";
  out[n++] =	"</form>";
...
function search(form) {
  ...
  if (term) {
    ...
    if (!result.length) {
      ...
      out[n++] =	"<table cellspacing='0' cellpadding='0' border='0'>\n"
      out[n++] =	"  <tr>\n"
      out[n++] =	"    <th class='l_edge'>KHz</th><th>Call</th><th>Location</th><th>DX("+get_units()+")</th><th>LSB</th><th>USB</th><th colspan='2'>Last RX</th><th>Logs</th>\n"
      out[n++] =	"  </tr>\n"
      for (var k in result) {
        var id =	result[k];
        out[n++] =	"  <tr title='Notes: " + station[id].notes + "'>\n"
        out[n++] =	"    <td class='l_edge'>" + station[id].khz + "</td>\n"
...
function popup_stats() {
  ...
  var out =	new Array();
  var n =	0;
  out[n++] =	"<!doctype html public '-//W3C//DTD HTML 4.01 Transitional//EN'>\n<html><head>\n<title>NDB WebLog for "+qth_name+" > Statistics</title>\n<META HTTP-EQUIV='Content-Type' CONTENT='text/html; charset=windows-1252'><link TITLE=\"new\" REL=\"stylesheet\" HREF=\"ndbweblog.css\" type=\"text/css\">\n</head>\n";
  ...
  if (monthly) {
    ...
    max_count['dx'] =	0; 
    max_count['cnt'] =	0;
    max_count['new_beacons'] =		0;
    max_count['new_beacons_ever'] =	0;
    max_count['rgn'] =	0;
    max_count['n60'] =	0;
    ...
    for (yyyy in stats.year) {
      ...
      if (stats.year[yyyy].new_beacons) {
    ...
    for (yyyy in stats.year) {
      ...
      if (stats.year[yyyy].new_beacons) {
        ...
        var new_beacons_ever =	new Array();
      ...
      // +++++++++++++++++++
        for (var c=0; c<12; c++) {
          var mm =	mm_arr[c];
          var val =	((stats.year[yyyy][mm] && stats.year[yyyy][mm].new_beacons)?(stats.year[yyyy][mm].new_beacons):(0))
          new_beacons[c] =	val
          if (val>max_count['new_beacons']) {
            max_count['new_beacons'] = val;
          }
          val = ((stats.year[yyyy][mm] && stats.year[yyyy][mm].new_beacons_ever)?(stats.year[yyyy][mm].new_beacons_ever):(0))
          new_beacons_ever[c] =	val
          if (val>max_count['new_beacons_ever']) {
            max_count['new_beacons_ever'] = val;
          }
        }
        ...
        out[n++] =	"<big><a name='"+yyyy+"new'></a>"+yyyy+" New Beacons Report (with UNIDs)"+link_top+"</big><br>\n";
        ...
        for (var c=0; c<12; c++) {
          var mm = mm_arr[c];
          out[n++] ="    <td bgcolor='"+get_graph_color(new_beacons_ever[c],max_count['new_beacons_ever'])+"' class='r'>"+((new_beacons_ever[c])?(new_beacons_ever[c]):("&nbsp;"))+"</td>\n";
        }
        out[n++] =	"    <th class='r'>"+stats.year[yyyy].new_beacons_ever+"</th>\n";
        out[n++] =	"  </tr>\n";
        ...
        for (var c=0; c<12; c++) {
          var mm = mm_arr[c];
          out[n++] ="    <td bgcolor='"+get_graph_color(new_beacons[c],max_count['new_beacons'])+"' class='r'>"+((new_beacons[c])?(new_beacons[c]):("&nbsp;"))+"</td>\n";
        }
        out[n++] =	"    <th class='r'>"+stats.year[yyyy].new_beacons+"</th>\n";
        out[n++] =	"  </tr>\n";
        out[n++] =	"</table>\n<p> </p>\n"
      }
      ...
      out[n++] =	"<tr><th class='l_edge_l'>Beacons</th>";
      for (var c=0; c<12; c++) {
        val =		dx_w[c]
      ...
      out[n++] =	"<table cellpadding='1' cellspacing='0' border='0'>\n  <tr>\n    <th width='120' class='l_edge'>Country (Sta)</th>\n"+month_columns+"<th colspan='2'>Best NDB<br>("+units_long+")</th>\n<th colspan='2'>Best DGPS<br>("+units_long+")</th>\n</tr>\n";

      for (b=0; b<countries.length; b++) {
        out[n++] =	"<tr>\n"
        out[n++] =	"<th class='l_edge_l'><a class='info' href='javascript:void 0' "
        ...
        out[n++] =	"<td bgcolor='"+get_graph_color(countries[b].mm[0], max_count['cnt'])+"' class='r'>"+(countries[b].mm[0]  ? countries[b].mm[0]  :"&nbsp;")+"</td>"+
        ...
        if (stats.year[yyyy] && stats.year[yyyy].cnt[sorted_cnt[b]]){
          ...
          out[n++] =	"<td nowrap>"+(typeof stats.year[yyyy].cnt[sorted_cnt[b]].best_ndb_id != "undefined" ? stats.year[yyyy].cnt[sorted_cnt[b]].best_ndb_id : "&nbsp;")+"</td>"
          out[n++] =	"<td>"
          ...
          out[n++] =	"</td>";
          out[n++] =	"<td nowrap>"+(typeof stats.year[yyyy].cnt[sorted_cnt[b]].best_dgps_id != "undefined" ? stats.year[yyyy].cnt[sorted_cnt[b]].best_dgps_id : "&nbsp;")+"</td>"
          out[n++] =	"<td>"
      ...
      out[n++] =	"<th class='r'>"+sorted_cnt.length+"</th>\n<th class='l'>" + (year_best_ndb_id ? year_best_ndb_id : "&nbsp;") + "</th>\n<th class='r'>" + (year_best_ndb_dx ? year_best_ndb_dx : "&nbsp;")+ "</th>\n<th class='l'>" + (year_best_dgps_id ? year_best_dgps_id : "&nbsp;") + "</th>\n<th class='r'>" + (year_best_dgps_dx ? year_best_dgps_dx : "&nbsp;")+ "</th>\n</tr>\n</table>\n<p> </p>\n\n"
      ...
      out[n++] =		"<big><a name='"+yyyy+"rr'></a>"+yyyy+" Regions Report (NDB and DGPS)"+link_top+"</big></br>\n";
      out[n++] =		"<table cellpadding='1' cellspacing='0' border='0'>\n  <tr>\n    <th width='120' class='l_edge'>&nbsp;</th>\n"+month_columns+"  </tr>\n";
...
function popup_text() {
  ...
  if (cookie['txt_date1']==cookie['txt_date2']) {
    ...
    if (cookie['txt_date1']!="19000001" && cookie['txt_date2']!="21000001") {
      switch (cookie['txt_format']) {
        ...
        case "yyyy-mm-dd": out[n++] =	"between "+start_yyyy+"-"+start_mm+"-"+start_dd+" and "+end_yyyy+"."+end_mm+"."+end_dd; break
        default:           out[n++] =	"between "+cookie['txt_date1']+" and "+cookie['txt_date2']; break
      }
    }
    if (cookie['txt_date1']!="19000001" && cookie['txt_date2']=="21000001") {
      switch (cookie['txt_format']) {
        ...
        case "yyyy-mm-dd": out[n++] =	"from "+start_yyyy+"-"+start_mm+"-"+start_dd+" onwards"; break
        default:           out[n++] =	"from "+cookie['txt_date1']+" onwards."; break
      }
    }
    if (cookie['txt_date1']=="19000001" && cookie['txt_date2']!="21000001") {
      switch (cookie['txt_format']) {
        ...
        case "yyyy-mm-dd": out[n++] =	"until "+end_yyyy+"-"+end_mm+"-"+end_dd; break
        default:           out[n++] =	"until "+cookie['txt_date2']+" onwards."; break
      }
    }
  }
  ...
  switch (cookie['txt_sortBy']) {
    ...
    case "gsq":  out[n++] = "Grid Square"; break;
...
//new TEXT(date,hhmm,khz,call,lsb,usb,pwr,dx,dxw,nu,qth,sta,cnt) {
      ...
      for (var b=0; b<logbook[id]['entry'].length; b++) {
        ...
        if (log_date>=start && log_date<=end &&
          ...
          if (cookie['txt_showAll']=='1' || !logbook[id]['shown']) {
            ...
            text_array[k++] =	new TEXT(temp_yyyymmddhhmm,temp_date,temp_hhmm,temp_khz,temp_call,temp_gsq,temp_lsb,temp_usb,temp_cyc,temp_pwr,temp_dx,temp_dxw,temp_nu,temp_qth,temp_sta,temp_cnt);
            logbook[id]['shown'] = true;
          }
        }
        else {
  ...
  for (var k=0; k<text_array.length; k++) {
    ...
    out[n++] =		(cookie['txt_h_cyc']!='1' ? pad(text_array[k].cyc,4)+" " : "");
    out[n++] =		(cookie['txt_h_pwr']!='1' ? pad(text_array[k].pwr,4)+" " : "");
    ...
    out[n++] =		(cookie['txt_h_gsq']!='1' ? text_array[k].gsq+" " : "");
...
function popup_text_options() {
  ...
  out[n++] =	"  var txt_h_cyc =    parseInt(document.form.txt_h_cyc[document.form.txt_h_cyc.selectedIndex].value);\n"
  out[n++] =	"  var txt_h_dxw =    parseInt(document.form.txt_h_dxw[document.form.txt_h_dxw.selectedIndex].value);\n"
  out[n++] =	"  var txt_h_gsq =    parseInt(document.form.txt_h_gsq[document.form.txt_h_gsq.selectedIndex].value);\n"
  ...
  out[n++] =	"  var txt_h_pwr =    parseInt(document.form.txt_h_pwr[document.form.txt_h_pwr.selectedIndex].value);\n"
  ...
  out[n++] =	"                txt_type+D+txt_lat1+D+txt_lat2+D+txt_lon1+D+txt_lon2+D+\n"
  out[n++] =	"                txt_h_cyc+D+txt_h_gsq+D+txt_h_pwr+\";expires=\"+expires;\n";
  out[n++] =	"  document.cookie = cookies;\n";
  out[n++] =	"  window.opener.popup_text();\n";
  out[n++] =	"  window.close();\n";
  out[n++] =	"}\n"
  out[n++] =	"</script>\n"
  out[n++] =	"</head>\n"
  ...
  out[n++] =	"  <tr>\n";
  out[n++] =	"    <td class='plain'><table cellpadding='0' cellspacing='0' border='0' class='r' width='100%'>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'>Text List > Output Options</th>\n";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap width='50%' class='l_edge'>Date Format&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>For each beacon&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>Sort By&nbsp;</td>\n";
  out[n++] =	"        <td nowrap><select name='txt_sortBy'>\n"
  out[n++] =	"<option value='call'" +(cookie['txt_sortBy']=='call' ? " selected" : "")+">Call</option>\n"
  out[n++] =	"<option value='cyc'"  +(cookie['txt_sortBy']=='cyc'  ? " selected" : "")+">Cycle</option>\n"
  ...
  out[n++] =	"<option value='gsq'"  +(cookie['txt_sortBy']=='gsq'  ? " selected" : "")+">Grid Square</option>\n"
  ...
  out[n++] =	"    <td class='plain'><table cellpadding='1' cellspacing='0' border='0' class='r' width='100%'>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'>Text List > Filters</th>";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap width='50%' class='l_edge'>Day / Night&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>NDB / DGPS&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>Dates (YYYYMMDD)&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>KHz&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>DX ("+get_units()+")&nbsp;</td>\n";
  ...
  out[n++] =	"Hint: for a North of 60 Report use 60 and All' class='l_edge'>Lat. (Decimal)&nbsp;</td>\n";
  ...
  out[n++] =	"Hint: to show stations West of 100, use All and -100' class='l_edge'>Lon. (Decimal)&nbsp;</td>\n";
  ...
  out[n++] =	"    <td class='plain'><table cellpadding='1' cellspacing='0' border='0' class='r' width='100%'>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'>Text List > Columns</th>";
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>Cycle&nbsp;</td>\n";
  out[n++] =	"        <td nowrap><select name='txt_h_cyc'>\n"
  out[n++] =	"<option value='0'" +(cookie['txt_h_cyc']!='1' ? " selected" : "")+">Show</option>\n"
  out[n++] =	"<option value='1'" +(cookie['txt_h_cyc']!='1' ? "" : " selected")+">Hide</option>\n\n"
  out[n++] =	"</select></td>\n"
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>DX per Watt&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>Grid Square&nbsp;</td>\n";
  out[n++] =	"        <td nowrap><select name='txt_h_gsq'>\n"
  out[n++] =	"<option value='0'" +(cookie['txt_h_gsq']!='1' ? " selected" : "")+">Show</option>\n"
  out[n++] =	"<option value='1'" +(cookie['txt_h_gsq']!='1' ? "" : " selected")+">Hide</option>\n\n"
  out[n++] =	"</select></td>\n"
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap class='l_edge'>Mod Offsets&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap width='50%' class='l_edge'>'New' loggings&nbsp;</td>\n";
  ...
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <td nowrap width='50%' class='l_edge'>Power&nbsp;</td>\n";
  out[n++] =	"        <td nowrap width='50%'><select name='txt_h_pwr'>\n"
  out[n++] =	"<option value='0'" +(cookie['txt_h_pwr']!='1' ? " selected" : "")+">Show</option>\n"
  out[n++] =	"<option value='1'" +(cookie['txt_h_pwr']!='1' ? "" : " selected")+">Hide</option>\n\n"
  out[n++] =	"</select></td>\n"
  out[n++] =	"      </tr>\n";
  out[n++] =	"      <tr>\n";
  out[n++] =	"        <th colspan='2' class='l_edge'><input type='button' value='Submit' onclick='go()'><input type='button' value='Cancel' onclick='window.close()'></th>";
  out[n++] =	"      </tr>\n";
  out[n++] =	"    </table></td>\n";
  out[n++] =	"  </tr>\n";
  out[n++] =	"</table>\n";
  out[n++] =	"</form>\n"
  text_options_h =		window.open('','zoneSelector', 'width=360,height=540,status=0,resizable=1,menubar=0,location=0,toolbar=0,scrollbars=0');
