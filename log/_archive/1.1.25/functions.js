  ...
  if (!get_cookie('popup_warning_given')) {
    ...
    if (eval(get_cookie('popup_warning_given'))>0) {
      ...
      switch (get_cookie('popup_warning_given')) {
        case "2": out+= "This message will be shown TWO more times\n(Sorry if it doesn't apply to you)."; break;
        case "1": out+= "This message will be shown ONE more time."; break;
        case "0": out+= "This message will not be shown again\n(unless you delete your cookies)"; break;
      }
      alert(out);
    }
  }
...
function popup_details(id) {
  ...
  var gsq=	((station[id].lat + station[id].lon !="")?
			 ("<a href='javascript:void window.opener.top.popup_map(\""+station[id].lat+"\",\""+station[id].lon+"\",\""+station[id].id+"\")' title='Click to show a map of this location'>"+station[id].gsq+"</a>"):
			 ("&nbsp;"));
  out[n++] =
    ...
    (station[id].rww!='' ? "  <td><a href='javascript:window.opener.top.rww("+station[id].rww+")'>RWW</a></td>" : "")+
  ...
  if (logbook[id]) {
    out[n++] =	"<h3>Reception Details for "+station[id].display+"</h3><table border='"+((document.all)?("0"):("1"))+"' cellspacing='0' cellpadding='1' border='0'>\n";
...
function popup_text() {
  ...
  switch (cookie['txt_type']) {
    case "dgps":   out[n++] = "DGPS station "; break;
    case "ndb":    out[n++] = "NDB "; break;
    case "navtex": out[n++] = "Naxtex station "; break;
    case "all":    out[n++] = "signal "
  }
...
//new TEXT(date,hhmm,khz,call,lsb,usb,pwr,dx,dxw,nu,qth,sta,cnt) {
  ...
  for (var k=0; k<text_array.length; k++) {
    ...
    out[n++] =
      (cookie['txt_h_cyc']!='1' ? pad(text_array[k].cyc,4)+" " : "")+
...
function rww(ID) {
  popWin('http://www.classaxe.com/dx/ndb/rww/?mode=signal_info&ID='+ID,'popsignal','scrollbars=0,resizable=1',640,380,'centre');
}
...
function list(yyyy,mm) {
  ...
  out[n++]=
    "<head><link TITLE='new' REL='stylesheet' HREF='ndbweblog.css' type='text/css'>\n"+
	"<style type='text/css'>\n"+
